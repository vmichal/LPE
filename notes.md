...
## Tıdny 9+10 - I2C magnetometr a SPI OLED displej
Instrukce pro tento tıden jsou na https://moodle.fel.cvut.cz/pluginfile.php/283750/mod_resource/content/5/LPE_9-10_tyden_poznamky_v1.pdf.

Pozn:
  - Jednotka 1 Gauss == 100 mikrotesla
  - Vektor magnetické intenzity zemského pole je v naší oblasti cca 48.5 mikrotesla.
  - Na OLED displeji je kadı pixel "elementární jednotka podsvicování" - spotøeba je pøímo úmìrná poètu svítících
   bodù
  - V displeji TFT mají pixely sdílené podsvícení a tak je odbìr relativnì konstantní bez ohledu na zobrazovanı obraz.

> 8.1 Pøipojte k MCU pomocí I2C sbìrnice 3-osı AMR magnetometr MMC5883, nahrajte do MCU dodanı bin soubor a
sledujte a popište princip komunikace po I2C sbìrnici. Jakı pøíkaz se posílá?
...
Sbìrnice I2C:
  ...
  Jejich velikost spoleènì s parazitní kapacitou vodièù vytváøí pøechodovı dìj,
    jeho rychlost omezuje baudrate sbìrnice.
  - Multi-slave prostøedí; jeden master øídí komunikaci a generuje hodiny.
  - Struktura rámce je vidìt níe
  na obrázku z pulseview: start condition, 7bitù adresy (vıbìr pøipojeného zaøízení), 1 bit na vıbìr typu
  rámce (read vs write), 1 bit acknowledge od kontaktovaného zaøízení.

Zachycenı a rozebranı rámec z komunikace se senzorem. Zaøízení s adresou 0x30 se zapisuje hodnota 0x10
do registru 0x08. Senzor pøíkaz pøijímá a všechny tøi bajty potvrzuje dominantní úrovní v ACK slotu:
![](week_9/i2c_frame.png)
Tentı rámec analyzovanı automaticky pomocí nástroje Pulseview a Embedded Logic Analyzer:
...
> 8.2 Vytvoøte pomocí dodané „knihovny“ program, kterı bude namìøené hodnoty magnetického pole B(nT) posílat do PC
a zobrazovat v terminálu. Jak vypadá vektor magnetického pole v Evropì (velikost, smìr)?

Zdrojovı kód je v souboru `code/week9/magnetometr_uloha_8.2.cpp` </br>
Kód pro odeètení elektrického offsetu mùstkového senzoru je v `code/week9/magnetometr_offset_remove.cpp`

Záznam vıpisu do terminálu se stabilním senzorem. Hodnoty jsou v nanoteslách, nejdøíve tøi sloky X, Y, Z a poté
...
Oèekávaná hodnota magnitudy B by mìla bıt cca 45 uT. Rotací senzoru ve všech smìrech se ukazuje, e bez další kalibrace spadají hodnoty
do pomìrnì širokého spektra hodnot mezi 35 uT a 60 uT. Následující vıpis ukazuje plynulé mìnìní celkové magnitudy pøi rotaci. 
```
...
```
> 8.3 Pøipojte grafickı OLED displej pøes SPI sbìrnici, sledujte a popište princip funkce SPI sbìrnice (pro zprovoznìní
pouijte dodanı bin soubor). Jakı pøíkaz se posílá – odpovídá chování displeje?

Zachycenı rámec komunikace na SPI, jednou na osciloskopu, jednou analyzováno strojovì pomocí pulseview:
![](week_9/spi_frame.png)
![](week_9/spi_pulseview.png)
Zachycenı je pøíkaz 0xA6, na støídaèku s ním se odesílá 0xA7. Podle datasheetu (https://cdn-shop.adafruit.com/datasheets/SSD1306.pdf)
to jsou pøíkazy pro pøepínání mezi normálním a invertovanım zobrazením.

> 8.4 Vytvoøte pomocí dodané „knihovny“ program, kterı bude namìøené hodnoty magnetického pole B(nT) zobrazovat
na OLED displeji.

Relevantní zdrojové kódy jsou ve sloce code/week9.

Originální kompas od katedry:

![](week_9/kompas.jpg)

Vlastní vıpis na displej:
...
![](week_7/schema72.jpg)
Vstupy AIN_R a AIN_B korespondují barvì kanálu na screenshotu z DataPlotteru. Èervenı kanál je signál z fototranzistoru,
modrı kanál je napìtí na senzoru teploty.

Screenshot pro ovìøení, e data pøichází pøesnì na 20 hertzích (èasová základna je nastavena tak, aby celı rozsah obrazovky 
...
Spojení signálovıch vodièù mezi MCU a wifi modulem (pozor na spojení zemí, napájení WiFi je z 5V
poblí USB konektoru na nepájivém poli):
![](week_7/schema73.jpg)
V tomto pøípadì se ještì na komunikaèní vodièe pøidaly rezistory 470 ohm na RX MCU a 47 ohm na TX MCU 

Zdrojovı kód je v mbedu pro úsporu èasu ve sloce `${root}/code/week7/LPE_wifi.cpp`.
Základní uivatelské prostøedí nakonec vypadá takto.
